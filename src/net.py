...
class BipartiteGNN(K.Model):
    ...
    def __init__(self, embedding_size=32, cons_num_features=2,
        ...
        self.cons_embedding = K.Sequential([
            K.layers.Dense(units=self.embedding_size, activation=self.activation,
        ...
        self.join_features_NN = K.Sequential([
            K.layers.Dense(units=self.embedding_size, activation=self.activation,
                           kernel_initializer=self.initializer),
            K.layers.Dense(units=self.embedding_size,
        ...
        self.output_module = K.Sequential([
            ...
            K.layers.Dense(units=self.embedding_size, activation=self.activation,
                           kernel_initializer=self.initializer),
            K.layers.Dense(units=1, activation=None,
                           kernel_initializer=self.initializer)
        ])
    ...
    def call(self, inputs):
        ...
        output_cons = self.cons_representation_NN(
            tf.concat([output_cons, cons_features], 1))
        ...
        output_vars = self.vars_representation_NN(
            tf.concat([output_vars, vars_features], 1))
        ...
        for batch in data:
            ...
            if train:
                with tf.GradientTape() as tape:
                    ...
                    loss = (loss * label.shape[0]) / total_0[0]
